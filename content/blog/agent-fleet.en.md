...
The most important convention: almost all of them are alert-only. Silence means everything is fine. A system that talks to you all the time is a system you learn to ignore.
...
Anyone can put an LLM on a cron job. What makes this work in production, with real fintech clients, is everything the agents can NOT do:

- **Fintech never auto-merges.** An explicit guard by repo name: any fintech client repo can only receive PRs. No exceptions, no flag to bypass it.
...
- Zero incidents caused by an agent. The guardrails are not decoration: each one exists because of something that happened, or something that cannot be allowed to happen.
- My manual operations time shrank to reviewing Telegram messages and approving or rejecting.

## What I learned

1. **Autonomy is earned in levels.** Observe, then diagnose, then write. Never the reverse.
2. **Alert-only or death.** The value of a monitoring system is measured by how little it talks when everything is fine.
3. **Humans pick the problems; agents execute.** My job stopped being typing code and became deciding what deserves attention and within which limits.
4. **Cheap guardrails prevent expensive incidents.** flock, a daily cap and a forbidden-repos regex are 20 lines of bash that make everything else possible.
