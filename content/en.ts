import type { Dictionary } from "./types"

export const en: Dictionary = {
  nav: {
    projects: "Projects",
    services: "Services",
    about: "About",
    blog: "Blog",
    links: "Links",
    contact: "Contact",
    cta: "Book a call",
  },
  hero: {
    eyebrow: "// Software developer · Fintech & product",
    role: "Custom software development",
    headline: "Custom software that grows your",
    headlineAccent: "business",
    lead: "I build SaaS, dashboards, automations and payment systems for founders and teams. From idea to production.",
    statement:
      "Nahuel Soria is a software developer based in Buenos Aires who builds SaaS, fintech payment systems and custom automations for founders and companies, end to end.",
    description:
      "I build SaaS, fintech payment systems and custom automations for founders and companies, end to end, from Buenos Aires. From idea to production.",
    bullets: [
      "Sites and landings that convert",
      "Scalable web apps and SaaS with data and payments",
      "Fintech integrations: KYC/KYB, settlement, multi-currency",
      "Internal process automation",
    ],
    ctaPrimary: "See projects",
    ctaSecondary: "Book a free intro call",
    availability: "Available for new projects",
  },
  proof: {
    eyebrow: "// Proof",
    title: "Real work, in production",
    reposLabel: "View on GitHub",
  },
  projects: {
    eyebrow: "// 01 · Projects",
    title: "Products I built end to end",
    subtitle:
      "Fintech platforms in production and my own products. Each with full technical ownership: architecture, backend, frontend and integrations.",
    roleLabel: "Role",
    stackLabel: "Stack",
    viewRepo: "Repo",
    viewLive: "Live",
    readCase: "Read the case study",
    statusLabel: {
      production: "In production",
      active: "Active",
      mvp: "MVP",
    },
  },
  caseStudy: {
    eyebrow: "// Case study",
    clientLabel: "Client",
    roleLabel: "Role",
    yearLabel: "Year",
    stackLabel: "Stack",
    visit: "Visit the site",
    backToProjects: "Back to projects",
    backToClients: "Back to services",
  },
  services: {
    eyebrow: "// 02 · Services",
    title: "How I can help",
    subtitle: "From a landing page to fintech infrastructure. We scope it around your goal.",
    featuredBadge: "Most chosen",
    ctaText: "Starting prices, what each channel includes and how payment works.",
    cta: "See pricing and scope",
  },
  process: {
    eyebrow: "// 03 · Process",
    title: "How I work",
    subtitle: "A clear process, with incremental delivery and constant communication.",
  },
  about: {
    eyebrow: "// 04 · About",
    title: "An engineer who thinks like a founder",
    statement:
      "I'm Nahuel Soria, a software developer and technical founder in Buenos Aires. I specialize in fintech (payments, KYC/KYB, settlement) and in custom SaaS products.",
    paragraphs: [
      "I work best where the problem is ambiguous, has to run in production, and can't be wrong. I'm the single technical owner behind several payment platforms: I own the architecture, the decisions and the delivery, not just the code.",
      "I also build my own products (Picaday, Numerai, What I Wish) and AI-powered code-auditing tools. I learn fast and solve whatever comes up.",
    ],
    stackTitle: "Core stack",
    principlesTitle: "How I work",
    principles: [
      "Correctness and auditability first",
      "End-to-end ownership, not just code",
      "Iterative, measurable delivery",
      "Simple decisions that scale",
    ],
    locationLabel: "Location",
    location: "Buenos Aires, Argentina · Remote worldwide",
  },
  faq: {
    eyebrow: "// 05 · FAQ",
    title: "Frequently asked questions",
    subtitle: "What people usually ask before we start.",
  },
  contact: {
    eyebrow: "// 06 · Contact",
    title: "Let's work together",
    subtitle:
      "Have a project in mind? Book a free 30-minute consultation and we'll assess your idea, no strings attached.",
    emailLabel: "Email",
    whatsappLabel: "WhatsApp",
    scheduleLabel: "Book a call",
    form: {
      name: "Name",
      email: "Email",
      message: "Message",
      messagePlaceholder: "Tell me about your project…",
      submit: "Send message",
    },
  },
  pricing: {
    eyebrow: "// Services and pricing",
    title: "Websites and custom software, with the starting price in plain sight",
    lead: "Two ways to work with me. A landing page or professional site delivered in two to three weeks, or custom software quoted by milestones. Both start with a free 30-minute call.",
    statement:
      "Nahuel Soria builds landing pages and professional websites from USD 1,500 and custom software (SaaS, internal systems, payment integrations) from USD 3,000 quoted by milestones, for companies worldwide, working remotely from Buenos Aires.",
    description:
      "Landing pages and websites from USD 1,500, custom software quoted by milestones and monthly maintenance. Free 30-minute call, remote from Buenos Aires.",
    channelsEyebrow: "// 01 · Two channels",
    channelsTitle: "Pick your way in",
    fromLabel: "from",
    deliveryLabel: "Delivery",
    includesLabel: "Includes",
    proofEyebrow: "// 02 · In production",
    proofTitle: "Sites delivered to clients",
    proofSubtitle: "Real work you can open right now. My own products and the fintech platforms are on the home page.",
    proofProductsLink: "See all projects",
    maintenanceEyebrow: "// 03 · After launch",
    maintenanceTitle: "Monthly maintenance",
    maintenanceText: "So the site keeps running without you having to think about it. Billed month to month, cancel whenever you want.",
    perMonth: "/ month",
    processEyebrow: "// 04 · Process",
    faqEyebrow: "// 05 · Pricing",
    faqTitle: "What people ask before hiring",
    faqSubtitle: "Scope, payments and what happens with what you already have.",
  },
  blog: {
    eyebrow: "// Blog",
    title: "Engineering notes",
    subtitle:
      "Real systems running in production: AI agents, fintech infrastructure and automation. No tutorials; what I build and what I learn operating it.",
    readingTime: "min read",
    back: "Back to blog",
  },
  links: {
    eyebrow: "// Links",
    title: "All my projects",
    subtitle: "Everything I build and where to find me, in one place.",
    description:
      "Everything I build and where to find me, in one place: the blog, the products I run in production and my GitHub, LinkedIn and X profiles.",
    groups: {
      writing: "Writing",
      project: "Projects",
      contact: "Contact",
    },
  },
  footer: {
    tagline: "Custom software development · Fintech & product",
    rights: "All rights reserved.",
    builtWith: "Built with Next.js",
  },
  theme: { toggle: "Toggle theme" },
  language: { toggle: "Switch language", label: "EN" },
}
